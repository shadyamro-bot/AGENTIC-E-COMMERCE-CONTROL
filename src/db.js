...
async function addColumn(table, definition) {
  await query(`ALTER TABLE ${table} ADD COLUMN IF NOT EXISTS ${definition}`);
}
...
async function initDb() {
  ...
  await query(`
    ...
    CREATE TABLE IF NOT EXISTS products (
      ...
      gender TEXT,
      category TEXT,
      brand TEXT NOT NULL DEFAULT 'Now Shoes',
      description TEXT,
  ...
  `);
  // Safe migrations for databases created by v1.0.
  ...
  if (count.rows[0].count === 0) {
    await query(`INSERT INTO users(name,email,role) VALUES
      ('Project Admin','admin@aec.local','ADMIN'),
      ('Listing Creator','creator@aec.local','CREATOR'),
      ('Quality Reviewer','reviewer@aec.local','REVIEWER'),
      ('Amazon Publisher','publisher@aec.local','PUBLISHER')`);
  } else {
    await query("UPDATE users SET email=LOWER(REPLACE(name,' ','_')) || '@aec.local' WHERE email IS NULL");
  }
